$(document).ready(function () {
  ...
  $(document).on("click", ".add", function () {
    ...
    html +=
      '<td><input type="text" name="item_code[]" id="productCode_' +
      count +
      '" value="' +
      count +
      '" class="form-control" autocomplete="off"></td>';
    html +=
      '<td><select name="item_name[]" id="productName_' +
      count +
      '" class="form-control select2 item_name" onchange="showHsn(' +
      count +
      ', this.value)" style="width:100% !important;"><option value="">Select Item</option></select></td>';

    html +=
      '<td><input type="text" name="item_desc[]" id="descName_' +
      count +
      '" class="form-control item_desc" /></td>';
    // html += '<td><input type="text" name="hsn[]" id="hsn_' + count + '" value="8443" class="form-control item_hsn" /></td>'; // HSN removed
    html +=
      '<td><input type="number" name="item_quantity[]" id="quantity_' +
      count +
    ...
    html +=
      ...
      count +
      '" class="form-control total" value="0" autocomplete="off"></td>';
    html +=
...
//     window.showHsn = function showHsn(str) {
//     if (str == "") {
//         $("#hsn_1").html("");
//         return;
//     }
...
//         success: function(response) {
//             // Check if response contains the HSN code
//             if (response.hsn) {
//                 // Update the HSN field with the returned HSN code
//                 $("#hsn_1").val(response.hsn); // Use .val() to update the input field
//             } else {
//                 // If no HSN code is returned, clear the HSN field
//                 $("#hsn_1").val('');
//             }
//         },
//         error: function(xhr, status, error) {
...
$("#form").submit(function (event) {
  ...
  $(".datarow").each(function () {
    ...
    // var hsnCode = $(this).find(".item_hsn"); // HSN removed
    ...
    // HSN validation removed as per requirements
    // if (!hsnCode.val()) {
  ...
  if (!supplier) {
    $("#supplier")
      ...
      .addClass("error"); // Fix for Select2
  ...
  if (hasError) {
    ...
    $(".datarow").each(function () {
      ...
      // hsn.push($(this).find(".item_hsn").val()); // HSN removed as per requirements
      ...
      success: function (response) {
        ...
        if (response.success) {
          ...
          Swal.fire({
            ...
            window.location.href = base_url + "/proinv/genproinv";
